...
aki:-pr_p_s, pr_pop_s, pr_gar_s, pr_pow_s, pr_gg_s, pr_past_s,
...
add(X1,X2,X3,X4,X5,X6):- write("Хорошо, ты меня победил. Я сдаюсь!\nВведите загаданного персонажа"),read(Q), asserta(pol(Q,X1)),
    asserta(popad(Q,X2)), asserta(garem(Q,X3)), asserta(power(Q,X4)), asserta(gg(Q,X5)), asserta(past(Q,X6)), reload,!.

reload:-pr_p_t, pr_pop_t, pr_gar_t, pr_pow_t, pr_gg_t, pr_past_t,
